...
For instance, the Mobile Monitoring Unit detects 40 grades of temperature, then it send the data to IoT agent. The Agent send the data to OCB following the air quality data model. The Activity component receives this data and the rule defined is: "if temperature > 40 grades then send an Heat Alert". Then this activity send the alert using the alert data model, and the description attribute could be added information as 241–54 °C Heat cramps and heat exhaustion are likely; heat stroke is probable with continued activity". The alert could be send to an application (alerts, routes), a smart phone, a web service, and so on.
