  ...
  compileSchema: function(fullPath,fileSchema,commonSchemas){
    ...
    try {
        ...
        if (typeof validate == 'function') {
          ...
          msg.addError(fullPath, 'Schema '+ file +' failed to compile to a function');
          if(options.getFailErrors()) throw new Error(validate.errors);
        }
    } catch (err) {
  ...
  validateExamples: function (fullPath,validate) {
    ...
    try{
      files.forEach(function(fileName){
        var data = schema.openFile(fileName, 'example ' + fileName);
        if (typeof validate != 'function') {
